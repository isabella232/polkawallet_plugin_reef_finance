...
import {BlockNumber, Nominations} from '@polkadot/types/interfaces';
...
async function loadValidatorRewardsData(api: ApiPromise, validatorId: string) {
  ...
  const points = _extractPoints(<any[]>stakerPoints);
  const rewards = _extractRewards(<any[]>erasRewards, <any[]>ownSlashes, <any[]>stakerPoints);
  const stakes = _extractStake(<any[]>ownExposure);
...
async function loadAccountRewardsData(api: ApiPromise, stashId: string, maxEras: number) {
  ...
  const filteredEras = (<any[]>allEras).slice(-1 * maxEras);
...
async function getAccountRewardsEraOptions(api: ApiPromise): Promise<EraSelection[]> {
  ...
  if (eraLength && historyDepth) {
    ...
    const maxBlocks = (<BlockNumber>eraLength).mul(historyDepth);
...
async function querySortedTargets(api: ApiPromise) {
 const data = await Promise.all([
  ...
  Promise.resolve([]),//getting error this method not defined - api.query.staking.minNominatorBond(),
 ]);
